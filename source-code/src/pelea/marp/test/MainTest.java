...
import pelea.atpa.common.enums.TF_TYPE;
import pelea.atpa.common.heuristic.config.CONFIG;
import pelea.atpa.service.ds.AnyTimePlanAdaptation;
...
public class MainTest {
	...
	private static String 	path_planes 			= "solutions";
	private static int 		planner_max_cpu_time 	= 1;
	
	
	/** BOTH CONFIGURATION */
	static ArrayList<DSPlan> 	input_plan; 
	static String[] 			domain 			= {"Rovers","logistics"};//,"DriverLog", "Woodworking", "openstacks"}; //,"Logistics","Satellite"}; 
	//static String[] 			domain 			= {"Rovers","logistics","DriverLog", "Woodworking", "openstacks"}; //,"Logistics","Satellite"}; 
	...
	static String[] 			benchmark 		= new String[7];	

	static String[] 			pddl31 			= new String[3]; // domain, and problem in PDDL
	static String[] 			pddl21 			= new String[3];

	static Conf configuration  = new Conf("pruebas-xml/benchmarks/configuration.xml");
	...
	static RGS_GH 				origin_pre			= null;
	static RGS_GH 				origin 				= null;
	...
	static boolean 				  debug_failures	= true;
	...
	public static void main(String[] args) {
		configuration_ds.getXml2java().getConfiguration();

		SingleSettingsImp settings = new SingleSettingsImp(configuration);
		
		SETTINGS_REACTIVESTRUCTURE mode_type = settings.getTypeExecution();
		...
		//String path = "/home/zenshi/Trabajos/java/Doctorado/ReactivePlanner/pruebas-xml/";
		//String path = "/home/anubis/Trabajos/java/doctorado/PELEA/ReactivePlanner/pruebas-xml/";
		String path = System.getProperty("user.dir");
		//benchmark[0] = "./pruebas-xml"; // folder Logistics mini pddl
		benchmark[0] = path+"/pruebas-xml/benchmarks/singleAgent/pddl3.1"; // folder pddl3.1
		// benchmark[0] = path+"/pruebas-xml/benchmarks/MultiAgent/pddl3.1"; // folder pddl3.1
		//benchmark[1] = "pddl"; // folder Logistics mini pddl
		benchmark[1] = "Pfile"; // folder Pfile pddl3.1
		//benchmark[2] = "domain3";// folder Logistics mini pddl
		benchmark[2] = "Domain.pddl";// name domain pddl3.1
		//benchmark[3] = "pfile3";// folder Logistics mini pddl
		benchmark[3] = "Problem.pddl";// name problem pddl3.1
		benchmark[4] = path+"/pruebas-xml/benchmarks/singleAgent/pddl2.1"; // folder pddl2.1
		// benchmark[4] = path+"/pruebas-xml/benchmarks/MultiAgent/pddl2.1"; // folder pddl2.1
		benchmark[5] = "domain.pddl";// name domain pddl2.1
		benchmark[6] = "pfile";// name problem pddl2.1
		...
		switch(mode_type){
			...
			case RP_TE_GENERATING_TREE_GRAPH1_MODE: 			results_generating_RS();		break;
	 ...
	 */
	private static void applicationExampleTreeGeneration() {
	 ...
	 */
	private static void marpTreeGeneration() {
		...
		if(input_plan.size()>=0){
			...
			// simulo la ejecución de las acciones hasta que queden dos acciones
			while(rp.getPlanningProblem().getBestPlan().getActionPlan().size()>2){
				rp.getPlanningProblem().simulateExecution(true);
			}
	 ...
	 * 01/08/2013
	 * Test for the Journal : Reactive Planner
	 * Generating the first three Repairing Structures (RS)
	 */
	private static void results_generating_RS() {
		...
		/** BOTH CONFIGURATION */
		int 	from_problem	= Integer.parseInt(configuration.getValue(From.C_REACTIVEPLANNER, "PROBLEM").split("-")[0]);
		int 	to_problem		= Integer.parseInt(configuration.getValue(From.C_REACTIVEPLANNER, "PROBLEM").split("-")[1]);;
		...
		for(int k=1;k<=op_test.total_number_executions;++k){
			...
			for(int time=1000;time<max_time;time=time+1000){
				...
				for (int j = 0; j < domain.length; ++j){
					...
					// for each problem
					for (int i = from_problem; i < to_problem; ++i){
						...
						results_generating_RS_TABLE_REPAIR_T(op_test);
					}
	...
	private static void results_generating_RS(TypeTests op_test2) {
		
        boolean debug_print_table = true;
        ...
        if(CONFIG.DEBUG_MET_STA) 
			CONFIG.PFILE = op_test2.problem;
		System.out.println("\ntask: "+op_test2.problem);
		
		
		
		
		
		// creating path of the pddl2.1 
		pddl21Path(op_test2.domain,op_test2.problem);
		
		// generating a solution plan or getting the last solution in the folder "solutions"
		input_plan 	= searchPlanToAction(pddl21, input_planner, n_sol_ori_plan, PLANNER.GET_LAST_SOLUTION);
		if(input_plan.size()>=0){
			
			// creating path of the pddl3.1 
			...
			((MAReactivePlanner)rp).translating(rp.getPlanningProblem().getPlan());
			...
			do{	
				...
				t_action = (int) ((MAReactivePlanner)rp)._trans_current_plan.size();
				System.out.println("\n\n[T"+no_search_tree+"] plan: "+rp.getPlanningProblem().getBestPlan().getActionPlan().size()+ " vs "+t_action +", ts: "+op_test2.res_limit_time_tree+"\n");
				
				
				
				
				// estimating the size of the repairing structure
				initialTime = System.currentTimeMillis();
				bestValue = rp.estimating_setting(op_test.res_limit_time_tree);
				
				bestValue = first_search_space(no_search_tree, op_test2);//*
				
				
				
				// calculating the repairing structure
				if(bestValue!=null){
					time = System.currentTimeMillis();
					rp.calculating(bestValue);
					op_test2.res_time_T = System.currentTimeMillis()-time;	
				}else System.out.println("It is not possible to generate the structure");
		        ...
		        else print(domain[op_test2.domain], op_test2);
				
				
				no_search_tree++;
				op_test2.setTimeLimitTree(op_test2.res_planning_horizon*op_test2.init_limit_time);
				
				
			}while(no_search_tree<=5 && !rp.is_plan_window_completed());
		}
	}
	...
	private static void results_generating_RS_TABLE_REPAIR_T(TypeTests op_test2) {
		...
		if(input_plan.size()>=0){
			...
			do{	
				...
				if(op_test2.problem == 4 || op_test2.problem == 5 || op_test2.problem == 8 || op_test2.problem == 10){
					repairing_TABLE_REPAIR_T(no_search_tree, op_test2);
				}else
					break;
	...
	private static void repairing_TABLE_REPAIR_T(int no_search_tree, TypeTests op_test2) {
	...
	private static EstimatedTime first_search_space(int no_search_tree, TypeTests op_test2) {
/*
		if(no_search_tree==1 && op_test2.problem==6)
			bestValue.setDepth(5);
		*/
		if(domain[op_test2.domain].toLowerCase().contains("rovers")){
			if(no_search_tree==1){
				if(op_test2.problem == 1){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(6);
				}
				if(op_test2.problem == 2){
					bestValue.setPlanningWindow(4);
					bestValue.setDepth(9);
				}
				if(op_test2.problem == 3){
					bestValue.setPlanningWindow(4);
					bestValue.setDepth(10);
				}
				if(op_test2.problem == 4){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(10);
				}
				if(op_test2.problem == 5){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(10);
				}
				...
				if(op_test2.problem == 7){
					bestValue.setPlanningWindow(3);
					bestValue.setDepth(9);
				}
				if(op_test2.problem == 8){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(7);
				}
				if(op_test2.problem == 9){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(6);
				}
				if(op_test2.problem == 10){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(8);
				}
				if(op_test2.problem == 11){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(7);
				}
				if(op_test2.problem == 12){
					bestValue.setPlanningWindow(2);
					bestValue.setDepth(7);
				}
			}
	 ...
	 */
	private static void verifyingPlanSearchTrees() {
	...
	private static void verifyingPlanSearchTrees(TypeTests op_test2) {
	...
	private static void printingPlanSearchTree(String domain, TypeTests op_test2) {
	 ...
	 */
	private static void testingGeneratingTreeGraph2() {
	...
	private static void testingTreeGenerationRPGraph2(TypeTests op_test2) {
		double 	timeGenerateT_i; 
		double 	stop;
	...
	private static void testingSimulatingFailureRP() {
	...
	private static void testingSimulatingFailureRP(TypeTests op_test2) {
		double 	timeGenerateT_i; 
		...
		if(input_plan.size()>=0){
			...
			bestValue = first_search_space(1, op_test2);//*
	...
	private static void testingSimulatingFailureRPVsDS() {
		...
		/** BOTH CONFIGURATION */
		int 	init_problem	= 1;
		int 	total_problem	= 11;
		int 	max_text_conf	= 2;
		int 	max_time		= 2000;
		/** BOTH CONFIGURATION */
		
		// for each test configuration
		for(int t=1;t<max_text_conf;++t){
			op_test = new TypeTests(TYPE_TEST.BYACT_TRGS_NOISY_ACTIONS_ONE.getType(t));/** Configuration of the test */
			op_test.technique=technique;			
			...
			for(int time=1000;time<max_time;time=time+1000){
				...
				for (int j = 0; j < domain.length; ++j){
					...
					/* old configuration of results for the decision support */
					if(technique == T_MODULES.MODULE_ALL || technique == T_MODULES.MODULE_ANYTIME){
						path_f_resul = op_test.path_eval_benchmarks+separator+"anyTime-"+domain[j]+"-"+type_domain[2]+"-"+input_planner+"-"+output_planner+".csv";
					}
					else if(technique == T_MODULES.MODULE_LPG_ADAPT){
						path_f_resul = op_test.path_eval_benchmarks+separator+"lpgAdapt-"+domain[j]+"-"+type_domain[2]+"-"+input_planner+"-"+output_planner+".csv";
					}
					...
					if(technique == T_MODULES.MODULE_ALL || technique == T_MODULES.MODULE_ANYTIME){
						evalDS 	= new Files(path_eval_ds, true);
						evalDS.write("\n");
						evalDS.close();
					}
	...
	private static void testingSimulatingFailureRPVsDS(TypeTests op_test2) {
		double 	timeGenerateT_i; 
		...
		if(input_plan.size()>=0){
			...
			/** initializing the Decision Support **/
			if(op_test2.technique == T_MODULES.MODULE_ALL || op_test2.technique == T_MODULES.MODULE_ANYTIME){
				anytime = new AnyTimePlanAdaptation(pddl31, TF_TYPE.TF_PDDL, input_plan, 
						currentInstantTime, "prueba", output_planner, configuration_ds);
			}					
			

			// executing just the anytime
			if(op_test2.technique == T_MODULES.MODULE_ANYTIME){	
				
				/** simulating a failure */ // the failure is simulated in the current state
				anytime.simulateFailure(T_FAILURE.BYACTION, op_test.noisy_actions);
				
				path_eval_ds = op_test.path_eval_benchmarks+"/anyTimeEvalDS-p"+CONFIG.PLANNER_LAMA+"-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;
				executingAnytime(pddl21,input_plan,path_f_resul,domain[op_test2.domain],op_test2.problem,n_sol_lpg_adapt,path_eval_ds);
			
			}else{
				
				// no se realiza ningún fallo
				if(op_test2.technique == T_MODULES.MODULE_LPG_ADAPT){
					
					path_eval_ds = op_test.path_eval_benchmarks+"/lpgAdaptEval-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;
					executingLPGADAPT(pddl21,input_plan,path_f_resul,domain[op_test2.domain],op_test2.problem,n_sol_lpg_adapt,path_eval_ds);
				
				}else{
							
					/** calculating the precompiled structure from the goals */
					if(op_test2.technique == T_MODULES.MODULE_ALL || op_test2.technique == T_MODULES.MODULE_REACTIVEPLANNER){
				        ...
				        // statictis
						// CONFIG.DEBUG_MET_STA 	= false;
						path_eval_rp	 = op_test.path_eval_benchmarks+"/evaluationsRP-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;
						path_analysis_rp = op_test.path_eval_benchmarks+"/analysisRP-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;										
						path_failures_rp = op_test.path_eval_benchmarks+"/failuresRP-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;
						
						evalRP2 = new Files(path_failures_rp, true);				
						evalRP2.write("\n**********************");
						evalRP2.write("\n"+domain[op_test2.domain]);
					...
					/*// statistics
					if(op_test2.technique == TYPET.MODULE_ALL){
						path_eval_ds	 = op_test.time+"/evaluationsDS-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;
						path_failures_ds = op_test.time+"/failuresDS-tf"+op_test.type_failure+"-f"+op_test.noisy_actions;

						evalDS2 = new Files(path_failures_ds, true);				
						evalDS2.write("\n**********************");
						evalDS2.write("\n"+domain[op_test2.domain]);
						evalDS2.write("\nWINDOW="+bestWindow.getWindow());
						evalDS2.write("\nDEPTH="+bestWindow.getDepth());
						evalDS2.close();
					}
			        
					
					* simulating a failure *
					simulatingFailure(op_test2.technique);
					++seed;
					
					
					/** printing the failure in a file *
					if(debug_failures) printingFailure(op_test2.technique, op_test2.problem);
	 ...
	 */
	private static void creatingDatasetBenchmarkMode() {
		...
		/** REACTIVE PLANNER CONFIGURATION */
		int 	planning_horizon_from 		= 3;
		int 	planning_horizon_to			= 8;
		int 	max_depth_actions			= 4;
		int 	start_from_action_plan		= 0;
		/** REACTIVE PLANNER CONFIGURATION */
		...
		for (int j = 0; j < domain.length; ++j){
			...
			for (int k = init_problem; k < total_problem; ++k){
				...
				if(input_plan.size()>=0){
					...
					/** initializing the Reactive Planner **/
					rp 	 = new MAReactivePlanner(pddl31, input_plan, configuration, "./results/");
					//rp.getPlanningProblem().printPlan();
					...
					do{
						System.out.println("[task"+k+"] total number of actions in the plan: "+t_action);
						
						// just to avoid to generate first repairing structures
						if(no_search_tree>start_from_action_plan){
						
							// we perform different windows and levels
							for(int PLANNINGHORIZON_SIZE=planning_horizon_from;PLANNINGHORIZON_SIZE<window_to;++PLANNINGHORIZON_SIZE){
								int max_depth = PLANNINGHORIZON_SIZE+max_depth_actions;
								for(int DEPTH_MAX=PLANNINGHORIZON_SIZE;DEPTH_MAX<=max_depth;++DEPTH_MAX){
									/** ELIMINAR **/
									if(PLANNINGHORIZON_SIZE==3 || PLANNINGHORIZON_SIZE==4 || PLANNINGHORIZON_SIZE==5)
										if(DEPTH_MAX == 4 || DEPTH_MAX == 5 || DEPTH_MAX == 6)
											continue;
									if(DEPTH_MAX==PLANNINGHORIZON_SIZE) continue;
									/** ELIMINAR **/
									
									//System.out.println("("+PLANNINGHORIZON_SIZE+","+DEPTH_MAX+") plan ("+t_action+") task ("+k+")");
									System.out.print("actions in the plan ("+t_action+") task"+k+" ");
									
									/** calculating the precompiled structure from the goals */
									((MAReactivePlanner)rp).translating();
									rp.calculating(new EstimatedTime(1000, PLANNINGHORIZON_SIZE, DEPTH_MAX,-1,-1,-1,null));
								}
							}
						}
					...
					}while(i<totalSimExec && t_action>0);
					...
					do{
						...
						System.out.println("[pfile"+k+"] total number of actions in the plan: "+t_action);
						...
						for(int PLANNINGHORIZON_SIZE=planning_horizon_from;PLANNINGHORIZON_SIZE<window_to;++PLANNINGHORIZON_SIZE){
							for(int DEPTH_MAX=PLANNINGHORIZON_SIZE;DEPTH_MAX<max_distance_to;++DEPTH_MAX){
								...
								System.out.println("("+PLANNINGHORIZON_SIZE+","+DEPTH_MAX+")");
	 ...
	 */
	private static void verifyingPlanSolutions() {
	...
	private static void replanningVsRepairing() {
		Simulator sim;
		String action;
		String name_pfile;
		String name_domain;
	...
	private static void verifyingResultsPartialStates() {
	...
	private static void printTable(String domain, TypeTests op_test2) {
        ...
        evalRP.write("\t"+op_test2.res_planning_horizon);
        evalRP.write("\t"+op_test2.res_depth);
        evalRP.write("\t"+op_test2.res_info_search_tree.generating_depth_reached);
        evalRP.write("\t"+op_test2.res_limit_time_tree);
	 ...
	 * @param timeStep 
	 */
	private static void simulatingFailure(T_MODULES technique, int timeStep) {
		...
		goal_state		= (ArrayList<DSCondition>) rp.get_goal_state(index_destiny).getFluents();
	...
	private static void executingAnytime(String[] pddl21,
	...
	private static void executingLPGADAPT(String[] pddl21,
	 ...
	 */
	private static ArrayList<DSPlan> searchPlanToAction(String[] dom_pro, int planToBeUsed, int n_sol, PLANNER last_sol) {
		...
		AnyPlanner original_plan = new AnyPlanner(dom_pro[0], dom_pro[1], n_sol, planner_max_cpu_time, "prueba", planToBeUsed, path_planes, dom_pro[2]);
		do{
